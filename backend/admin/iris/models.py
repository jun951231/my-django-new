...
class Iris(object):
    ...
    def base(self):
        ...
        '''
        ['sepal length (cm)', 꽃바침 조각
         'sepal width (cm)', 꽃바침 조각
         'petal length (cm)', 꽃잎
         'petal width (cm)'] 꽃잎
        '''

        iris_df['species'] = pd.Categorical.from_codes(iris.target, iris.target_names)
        # print(f'추가된 아이리스 데이터 구조: {iris_df.head(2)} \n {iris_df.columns}')
        '''
        ...
        features = iris_df.columns[:4] # 0-3까지 fearture 추출
        # print(f'아이리스 features 값: {features} \n')
        y = pd.factorize(train['species'])[0]
        # print(f'아이리스 y 값: {y}') # 총 3종류 품종
