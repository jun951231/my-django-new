...
class CrimeCctvModel(object):
    ...
    def create_new_police(self):
        vo =self.vo
        reader =self.reader
        printer = self.printer
        vo.context = 'admin/crime/data/new_data/'
        vo.fname = 'police_position'
        police_file_name = reader.new_file(vo)
        # print(f'파일명: {police_file_name}')
        new_police = reader.csv(police_file_name, 1, 'C13')
        new_police.columns = ['구별']
        printer.dframe(new_police)
        new_police.to_csv(vo.context + 'new_police.csv')
        return new_police
    ...
    def create_population_model(self):
        ...
        pop.drop([26], inplace=True) # 26번째 줄 없애기
        printer.dframe(pop)
